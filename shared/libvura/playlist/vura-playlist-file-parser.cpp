...
{
public:
    explicit ParserBase(PlaylistFileParser *parent) : m_parent(parent), m_aborted(false)
    {
        Q_ASSERT(m_parent);
    }

    bool parseLine(const int lineIndex, const QString &line, const QUrl &root)
    {
        if (m_aborted)
            return false;
...
protected:
    ...
    void newItemFound(const QVariant &content) const { Q_EMIT m_parent->newItem(content); }
...
{
public:
    explicit M3UParser(PlaylistFileParser *q) : ParserBase(q), m_extendedFormat(false) { }
     ...
     */
    bool parseLineImpl(const int lineIndex, const QString &line, const QUrl &root) override
    {
        if (line[0] == u'#') {
            if (m_extendedFormat) {
...
{
public:
    explicit PLSParser(PlaylistFileParser *q) : ParserBase(q) { }
...
{
    Q_DECLARE_PUBLIC(PlaylistFileParser)
public:
    PlaylistFileParserPrivate(PlaylistFileParser *q)
        : q_ptr(q),
          m_stream(nullptr),
          m_type(PlaylistFileParser::UNKNOWN),
          m_scanIndex(0),
          m_lineIndex(-1),
          m_utf8(false),
          m_aborted(false)
    ...
    QString m_mimeType;
    PlaylistFileParser *q_ptr;
    QPointer<QIODevice> m_stream;
    PlaylistFileParser::FileType m_type;
    struct ParserJob
    {
...
bool PlaylistFileParserPrivate::processLine(int startIndex, int length)
{
    Q_Q(PlaylistFileParser);
    m_lineIndex++;
    ...
    if (!m_currentParser) {
        ...
        m_type = PlaylistFileParser::findPlaylistType(
                suffix, !mimeType.isEmpty() ? mimeType : m_mimeType, m_buffer.constData(),
                quint32(m_buffer.size()));

        switch (m_type) {
        case PlaylistFileParser::UNKNOWN:
            emit q->error(Playlist::FormatError,
                          Playlist::tr("%1 playlist type is unknown").arg(m_root.toString()));
            q->abort();
            return false;
        case PlaylistFileParser::M3U:
            m_currentParser.reset(new M3UParser(q));
            break;
        case PlaylistFileParser::M3U8:
            m_currentParser.reset(new M3UParser(q));
            m_utf8 = true;
            break;
        case PlaylistFileParser::PLS:
...
void PlaylistFileParserPrivate::handleData()
{
    Q_Q(PlaylistFileParser);
    ...
    while (m_stream->bytesAvailable() && !m_aborted) {
        ...
        if (m_buffer.length() - processedBytes >= LINE_LIMIT) {
            emit q->error(Playlist::FormatError,
                          Playlist::tr("invalid line in playlist file"));
            q->abort();
            break;
        }
...
PlaylistFileParser::PlaylistFileParser(QObject *parent)
    : QObject(parent), d_ptr(new PlaylistFileParserPrivate(this))
{
}

PlaylistFileParser::~PlaylistFileParser() = default;

PlaylistFileParser::FileType PlaylistFileParser::findByMimeType(const QString &mime)
{
    if (mime == QLatin1String("text/uri-list") || mime == QLatin1String("audio/x-mpegurl")
        || mime == QLatin1String("audio/mpegurl"))
        return PlaylistFileParser::M3U;

    if (mime == QLatin1String("application/x-mpegURL")
        || mime == QLatin1String("application/vnd.apple.mpegurl"))
        return PlaylistFileParser::M3U8;

    if (mime == QLatin1String("audio/x-scpls"))
        return PlaylistFileParser::PLS;

    return PlaylistFileParser::UNKNOWN;
}

PlaylistFileParser::FileType PlaylistFileParser::findBySuffixType(const QString &suffix)
{
...
PlaylistFileParser::FileType PlaylistFileParser::findByDataHeader(const char *data, quint32 size)
{
    if (!data || size == 0)
        return PlaylistFileParser::UNKNOWN;
...
PlaylistFileParser::FileType PlaylistFileParser::findPlaylistType(const QString &suffix,
                                                                    const QString &mime,
 ...
 * Delegating
 */
void PlaylistFileParser::start(const QUrl &media, QIODevice *stream, const QString &mimeType)
{
    if (stream)
...
void PlaylistFileParser::start(QIODevice *stream, const QString &mimeType)
{
    Q_D(PlaylistFileParser);
    const bool validStream = stream ? (stream->isOpen() && stream->isReadable()) : false;

    if (!validStream) {
        Q_EMIT error(Playlist::AccessDeniedError, Playlist::tr("Invalid stream"));
        return;
    }
...
void PlaylistFileParser::start(const QUrl &url, const QString &mimeType)
{
    Q_D(PlaylistFileParser);

    if (url.isLocalFile() && !QFile::exists(url.toLocalFile())) {
        emit error(Playlist::AccessDeniedError,
                   QString(Playlist::tr("%1 does not exist")).arg(url.toString()));
        return;
    }
...
void PlaylistFileParser::handleData()
{
    Q_D(PlaylistFileParser);
    d->handleData();
}

void PlaylistFileParserPrivate::handleParserFinished()
{
    Q_Q(PlaylistFileParser);
    const bool isParserValid = bool(m_currentParser);
    if (!isParserValid && !m_aborted)
        emit q->error(Playlist::FormatNotSupportedError,
                      Playlist::tr("Empty file provided"));
...
void PlaylistFileParserPrivate::abort()
{
    m_aborted = true;
    if (m_currentParser)
...
void PlaylistFileParserPrivate::reset()
{
...
void PlaylistFileParser::handleError()
{
    Q_D(PlaylistFileParser);
    const QString &errorString = d->m_source->errorString();
    Q_EMIT error(Playlist::NetworkError, errorString);
    abort();
}
