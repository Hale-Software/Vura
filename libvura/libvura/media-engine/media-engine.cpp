...
VuraMediaEngine::VuraMediaEngine(QWidget *parent) : QWidget(parent), m_state(Stopped)
{
    // Setup the UI layout to hold the OpenGL surface
    ...
    connect(m_videoWidget, &VideoWidget::positionChanged, this, &VuraMediaEngine::onPositionChanged);

    initializePipeline();
}

VuraMediaEngine::~VuraMediaEngine()
{
    stop();
    cleanupPipeline();
}
...
void VuraMediaEngine::loadMedia(const QString& filePath)
{
    ...
    connect(m_demuxer, &DemuxerThread::videoDimentionsFound, this, &VuraMediaEngine::onVideoDimensionsFound, Qt::QueuedConnection);

    if (!m_demuxer->openFile(filePath)) {
        qWarning() << "VuraMediaEngine: failed to open" << filePath;
        return;
    }
    ...
    if (!m_videoDecoder->initDecoder()) {
        qWarning() << "VuraMediaEngine: video decoder init failed";
        return;
    }
    ...
    if (aIdx >= 0) {
        ...
        if (m_audioDecoder->initDecoder() && m_audioOutput->init()) {
            ...
            qDebug() << "VuraMediaEngine: audio pipeline ready";
        } else {
            qWarning() << "VuraMediaEngine: audio init failed, falling back to wall clock";
...
void VuraMediaEngine::loadMedia(const QUrl& url)
{
    stop(); // Ensure previous media is cleared
    cleanupPipeline();
    initializePipeline();
    ...
    if (!m_demuxer->openFile(url.toLocalFile())) {
        qWarning() << "VuraMediaEngine: failed to open" << url.toLocalFile();
        return;
    }
...
void VuraMediaEngine::seek(const int seconds) const
{
    if (!m_demuxer) return;
...
void VuraMediaEngine::setPlaybackRate(double rate)
{
    if (rate <= 0.0) return;
    m_playbackRate = rate;
...
bool VuraMediaEngine::isPlaying() const { return m_isPlaying; }
int  VuraMediaEngine::duration()  const { return m_duration; }
int  VuraMediaEngine::position()  const { return m_position; }

// ---------------------------------------------------------------------------
// Private slots
// ---------------------------------------------------------------------------

void VuraMediaEngine::onPositionChanged(const int seconds)
{
    m_position = seconds;
    emit positionChanged(seconds);
}

void VuraMediaEngine::onVideoDimensionsFound(const int width, const int height)
{
...
void VuraMediaEngine::initializePipeline()
{
...
void VuraMediaEngine::cleanupPipeline()
{
    // 1. Stop render timer and audio output before touching queues
    m_videoWidget->pause();
    if (m_audioOutput) m_audioOutput->setPaused(true);

    // FIX: Disconnect the audio clock so VideoWidget doesn't use a dangling pointer
    m_videoWidget->setAudioClock(nullptr);

    // FIX: Request interruption on ALL threads BEFORE waking them up
    // This ensures they exit immediately when the queue abort unblocks them.
    if (m_demuxer)      m_demuxer->requestInterruption();
    if (m_videoDecoder) m_videoDecoder->requestInterruption();
    if (m_audioDecoder) m_audioDecoder->requestInterruption();
    if (m_audioOutput)  m_audioOutput->requestInterruption();
    ...
    auto stopThread = [](QThread* t) {
        if (!t) return;
        // t->requestInterruption(); <-- Removed, already done globally above
        t->quit();
        t->wait();
    };
