...
#include "playlist-file-parser.h"
...
bool PlaylistFileParserPrivate::processLine(int startIndex, int length)
{
    ...
    if (!m_currentParser) {
        ...
        case PlaylistFileParser::UNKNOWN:
            emit q->error(Playlist::FormatError, Playlist::tr("%1 playlist type is unknown").arg(m_root.toString()));
            q->abort();
            return false;
...
void PlaylistFileParserPrivate::handleData()
{
    ...
    while (m_stream->bytesAvailable() && !m_aborted) {
        ...
        if (m_buffer.length() - processedBytes >= LINE_LIMIT) {
            emit q->error(Playlist::FormatError, Playlist::tr("invalid line in playlist file"));
            q->abort();
            break;
        }
...
PlaylistFileParser::PlaylistFileParser(QObject *parent) : QObject(parent), d_ptr(new PlaylistFileParserPrivate(this)) {}
...
PlaylistFileParser::FileType PlaylistFileParser::findPlaylistType(const QString &suffix, const QString &mime, const char *data, quint32 size)
{
...
void PlaylistFileParser::start(const QUrl &request, const QString &mimeType)
{
    Q_D(PlaylistFileParser);
    const QUrl &url = request.url();

    if (url.isLocalFile() && !QFile::exists(url.toLocalFile())) {
        emit error(Playlist::AccessDeniedError, QString(Playlist::tr("%1 does not exist")).arg(url.toString()));
        return;
    }
...
void PlaylistFileParserPrivate::handleParserFinished()
{
    ...
    if (!isParserValid && !m_aborted)
        emit q->error(Playlist::FormatNotSupportedError, Playlist::tr("Empty file provided"));
