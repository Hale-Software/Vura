...
VideoPlayerWidget::VideoPlayerWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(m_videoSink, &QVideoSink::videoFrameChanged, this, &VideoPlayerWidget::handleFrame);

    m_rect = QRect((this->width() / 2), 15, (this->width() - 30), 50);
}

void VideoPlayerWidget::setSource(const QUrl &source)
{
...
void VideoPlayerWidget::setAudioOutput(QAudioOutput *audioOutput)
{
    m_audioOutput = audioOutput;
}

void VideoPlayerWidget::handleFrame(const QVideoFrame &frame)
{
    if (frame.isValid()) {
...
void VideoPlayerWidget::subtitleTextChanged(const QString &subtitleText)
{
    //m_subtitleText = subtitleText;
    //update();
}

void VideoPlayerWidget::togglePlayback() const
{
