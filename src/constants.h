...
inline QString MediaFileExtensions = R"(Media Files (
 ...
 *.rpl *.thp *.tod *.tp *.ts *.tts *.txd *.vob *.vprj *.vro *.webm *.wm *.wmv *.wtv *.xesc *.3ga *.669 *.a52 *.aac
 *.ac3 .adt *.adts *.aif *.aifc *.aiff *.amb *.amr *.aob *.ape *.au *.awb *.caf *.dts *.flac *.it *.kar *.m4a *.m4b
 *.m4p *.m5p *.mid *.mka *.mlp *.mod *.mpa *.mp1 *.mp2 *.mp3 *.mpc *.mpga *.mus *.oga *.ogg *.oma *.opus *.qcp *.ra
 *.rmi *.s3m *.sid *.spx *.tak *.thd *.tta *.voc *.vqf *.w64 *.wav *.wma *.wv *.xa *.xm *.asx *.b4s *.cue *.hlist *.ifo
 *.m3u *.m3u8 *.pls *.ram *.rar *.sdp *.vlc *.vpl *.xspf *.wax *.wvx *.zip *.conf)
)";

inline QString VideoFileExtensions = R"(Video Files (
*.3g2 *.3gp *.3gp2 *.3gpp *.amv *.asf *.avi *.bik *.bin *.crf *.dav *.divx *.drc *.dv *.dvr-ms *.evo *.f4v *.flv
 *.gvi *.gxf *.iso *.m1v *.m2v *.m2t *.m2ts *.m4v *.mkv *.mov *.mp2 *.mp2v *.mp4 *.mp4v *.mpe *.mpeg *.mpeg1
 *.mpeg2 *.mpeg4 *.mpg *.mpv2 *.mts *.mtv *.mxf *.mxg *.nsv *.nuv *.ogg *.ogm *.ogv *.ogx *.ps *.rec *.rm *.rmvb
...
inline QString AudioFileExtensions = R"(Audio Files (
...
inline QString PlaylistFileExtensions = R"(Playlist Files (
