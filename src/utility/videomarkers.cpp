...
VideoMarkers::VideoMarkers(QObject *parent) : QObject{parent}
{
    ...
    if (!loadVideoMarkersFile(settings.value("markerFile", defaultMarkerFile).toString())) {
        qWarning() << "Failed to load video markers file from settings.";

        if (createBlankVideoMarkersFile(settings.value("markerFile", defaultMarkerFile).toString())) {
            qDebug() << "Created blank video markers file.";

            if (!loadVideoMarkersFile(settings.value("markerFile", defaultMarkerFile).toString())) {
                qCritical() << "Failed to load blank video markers file.";

            }
        } else {
            qCritical() << "Failed to create blank video markers file.";
        }
    }
}
...
QList<double> VideoMarkers::markersTypeList(const QJsonValue &markerValue)
{
    ...
    if (markerValue.isArray()) {
        ...
        for (const QJsonValue &value : jsonArray) {
            if (value.isDouble()) {
                returnList.append(value.toDouble());
            }
        }
    }
...
bool VideoMarkers::createBlankVideoMarkersFile(QString filename)
{
...
bool VideoMarkers::loadVideoMarkersFile(QString filename)
{
    QFile dataFile(filename);
    if (!dataFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qCritical() << "Could not read video markers file: " << dataFile.errorString();
        return false;
    }

    QJsonDocument doc = QJsonDocument::fromJson(dataFile.readAll());
    if (doc.isNull()) {
        qCritical() << "Failed to parse JSON document in video markers file";
        return false;
    }

    if (!doc.isObject()) {
        qCritical() << "JSON document is not an object in video markers file";
        return false;
    }
...
bool VideoMarkers::saveVideoMarkersFile(QString filename)
{
