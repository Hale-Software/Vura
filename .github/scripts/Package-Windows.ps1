...
if ( ! ( [System.Environment]::Is64BitOperatingSystem ) ) {
    throw "vura requires a 64-bit system to build and run."
}

if ( $PSVersionTable.PSVersion -lt '7.2.0' ) {
    Write-Warning 'The vura packaging script requires PowerShell Core 7. Install or upgrade your PowerShell version: https://aka.ms/pscore6'
    exit 2
}
...
function Package {
    ...
    Log-Group "Packaging vura..."

    Push-Location -Stack PackageTemp "build_${Target}"

    cpack @CpackArgs

    $Package = Get-ChildItem -filter "vura-*-windows-${Target}.zip" -File
